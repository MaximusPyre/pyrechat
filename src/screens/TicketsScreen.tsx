...
import { ApiError, api, apiOrigin, getToken, uploadTicketFile } from "../lib/api";
		...
		return (
			<div className="overlay-page">
				<div className="page-head">
					...
					<button className="icon-btn solid" onClick={onBack} aria-label="Close">
			...
			<div className="settings">
				<div className="card">
					<p className="muted" style={{ margin: 0, fontWeight: 600, lineHeight: 1.45 }}>
						File a real bug or a feature. During private beta a builder bot audits it, implements it if it is viable, then writes the result back to your account. Attach screenshots or files. Nothing auto-merges.
					</p>
				</div>
				<div className="card">
					<h3>New ticket</h3>
					<select className="select" value={kind} onChange={(e) => setKind(e.target.value as "bug" | "feature")}>
				...
				{tickets.map((t) => (
					<div key={t.id} className="card">
						<div className="muted" style={{ fontWeight: 800, letterSpacing: "0.08em", fontSize: 11 }}>
							{t.kind.toUpperCase()} · {STATUS[t.status] || t.status}
							{founder && t.username ? ` · @${t.username}` : ""}
						</div>
						<h3 style={{ margin: "6px 0" }}>{t.title}</h3>
						<p className="muted" style={{ margin: 0, whiteSpace: "pre-wrap", fontWeight: 600 }}>
							{t.body}
						</p>
						{(t.attachments || []).length > 0 && (
							<div className="ticket-files">
								{t.attachments.map((a) =>
									a.image ? (
										<a key={a.id} href={authedPreview(a.url)} target="_blank" rel="noreferrer" className="ticket-file">
											<img src={authedPreview(a.url)} alt={a.name} />
										</a>
									) : (
										<a key={a.id} href={authedPreview(a.url)} target="_blank" rel="noreferrer" className="ticket-file ticket-file-doc">
				...
				{tickets.length === 0 && <div className="empty">No tickets yet. First one through the door sets the tone.</div>}
			</div>
		</div>
	);
}

function authedPreview(path: string): string {
