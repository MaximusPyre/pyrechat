...
export function gradeCss(id: GradeId): string {
...
export function drawVideoFrame(
	...
	opts: { mirror: boolean; grade: GradeId },
): void {
	...
	ctx.filter = gradeCss(opts.grade);
...
export function drawCaptureFrame(
	...
	drawVideoFrame(ctx, video, opts);
	if (!pts || opts.lens === "none") return;
	drawLens(ctx, pts, ctx.canvas.width, ctx.canvas.height, opts.lens);
}
